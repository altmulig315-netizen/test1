...
def dekrypter_med_md5(filnavn, passord):
    ...
    # Generer nøkkel og IV med MD5 (OpenSSL metode for AES-256-CBC)
    ...
    # Første runde: MD5(password + salt)
    d = hashlib.md5(password_bytes + salt).digest()
    
    # Andre runde: MD5(d + password + salt)
    d2 = hashlib.md5(d + password_bytes + salt).digest()
    
    # Nøkkel for AES-256 (32 bytes)
    key = d + d2
    
    # Tredje runde: IV = MD5(d2 + password + salt)
    iv = hashlib.md5(d2 + password_bytes + salt).digest()
